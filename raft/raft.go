...
func (r *Raft) appendEntries(req *pb.AppendEntriesRequest) (*pb.AppendEntriesResponse, error) {
	...
	} else if term == r.currentTerm && r.state != Follower {
		//-----------------------------------------------------------------------------------------------------------------------------
	...
	if leaderCommitId > r.commitIndex {
		id, _ := r.getLastLog()
		if leaderCommitId < id {
			r.setCommitIndex(leaderCommitId)
			// r.commitIndex = leaderCommitId
		} else {
			r.setCommitIndex(id)
			// r.commitIndex = id
		}
...
func (r *Raft) broadcastRequestVote(ctx context.Context, voteCh chan *voteResult) {
	...
	lastLogId, lastTerm := r.getLastLog()
...
func (r *Raft) broadcastAppendEntries(ctx context.Context, appendEntriesResultCh chan *appendEntriesResult) {
	...
	for peerId, peer := range r.peers {
		...
		r.logger.Debug("send append entries", zap.Uint32("peer", peerId), zap.Any("request", req), zap.Int("entries", len(req.Entries)))
		// TODO: (A.14) & (B.6)
